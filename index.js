...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'Denara Bot', 'ubuntu', '3.0' ]
...
kurr.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6285727492435@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `DenaraBotz-Assistan`, img,but = [{buttonId:`donasi`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `DenaraBotz-Assistan`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `SAYONARA !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6285727492435@s.whatsapp.net","6285727492435@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6285727492435@s.whatsapp.net"
